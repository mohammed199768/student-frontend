                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        <div>
                            ...
                            <div className="relative">
                                <Mail className="absolute left-3 top-3.5 h-5 w-5 text-slate-500 dark:text-slate-400 rtl:right-3 rtl:left-auto" />
                                <input
                                    {...register('email')}
                                    type="email"
                                    placeholder="name@example.com"
                                    className="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 py-3 pl-10 pr-4 text-slate-900 dark:text-white focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 rtl:pr-10 rtl:pl-4"
                                />
                            </div>
                        ...
                        <div>
                            <div className="mb-2 flex items-center justify-between">
                                <label className="text-sm font-bold text-slate-700 dark:text-slate-200">{t('password')}</label>
                                <Link href="/forgot-password" className="text-xs font-bold text-indigo-600 hover:underline">{t('forgot_password')}</Link>
                            </div>
                            <div className="relative">
                                <Lock className="absolute left-3 top-3.5 h-5 w-5 text-slate-500 dark:text-slate-400 rtl:right-3 rtl:left-auto" />
                                <input
                                    {...register('password')}
                                    type={showPassword ? 'text' : 'password'}
                                    placeholder="********"
                                    className="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 py-3 pl-10 pr-10 text-slate-900 dark:text-white focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 rtl:pr-10 rtl:pl-10"
                                />
                                <button
                                    type="button"
