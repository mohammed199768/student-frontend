/**
 * Unified Client Logger for LMS Student Frontend
 * Provides structured console logging with automatic sanitization.
 * SSR-safe: checks for window before browser-specific operations.
 * 
 * Features:
 * - Consistent prefix: [LMS-STUDENT]
...
function formatMessage(level: LogLevel, message: string): string {
    return `[LMS-STUDENT][${level}][${formatTimestamp()}] ${message}`;
}
